...
export default function Search() {
  ...
  return <div className="p-8 pb-32 max-w-5xl mx-auto"><div className="mb-6 flex gap-3"><div className="relative flex-1"><SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground"/><Input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Rechercher dans le catalogue iTunes…" className="w-full pl-12 h-14 bg-white/5 border-white/10 text-lg rounded-xl"/></div><Button variant="outline" className="h-14" onClick={()=>fileInput.current?.click()} disabled={busy}><Upload className="w-4 h-4 mr-2"/>Importer un audio</Button><input ref={fileInput} className="hidden" type="file" accept="audio/mpeg,audio/mp4,audio/aac,audio/wav,audio/x-wav,audio/ogg,audio/webm,audio/flac" onChange={e=>e.target.files?.[0]&&void upload(e.target.files[0])}/></div><p className="text-xs text-muted-foreground mb-6">Aperçus et liens d’achat fournis par iTunes Store. Les BPM, tonalités et énergies inconnus restent inconnus.</p>{query.length>=2&&<section className="mb-10"><h2 className="font-serif text-xl text-white mb-3">Catalogue {busy?'…':''}</h2>{results.map(t=><div key={t.id} className="flex items-center gap-3 border-b border-border py-2"><img className="w-10 h-10 rounded object-cover" src={t.cover || '/favicon.svg'} alt=""/><div className="flex-1 min-w-0"><p className="text-sm text-white truncate">{t.title}</p><p className="text-xs text-muted-foreground">{t.artist}</p></div>{t.storeUrl&&<a className="text-xs text-primary" href={t.storeUrl} target="_blank" rel="noreferrer">iTunes</a>}<Button size="sm" onClick={()=>void add(t)}>Ajouter</Button></div>)}</section>}<section><h2 className="font-serif text-xl text-white mb-3">Votre bibliothèque</h2>{library.length?library.map(t=><TrackRow key={t.id} track={t}/>):<p className="text-muted-foreground py-12 text-center border border-dashed border-border rounded-xl">Votre bibliothèque est vide. Cherchez un aperçu autorisé ou importez un audio dont vous détenez les droits.</p>}</section></div>;
}